...
该包遵循 Agent Skills 格式的 `SKILL.md` + 可选脚本/资源结构。开放规范定义包格式，不定义发现目录、调用语法或工具权限。
...
[`INSTALL.md`](INSTALL.md) 保留了简短安装说明。优先使用宿主内置的 Skill 安装能力；已有同名目录时先保留可恢复备份，不静默覆盖用户修改。

安装后开启新任务并在技能列表中确认 `delivery-gate`。目录存在表示文件已复制，列表可见表示宿主已发现；两者都不能单独证明当前回复已经激活了 Skill。
...
这些是不依赖原生发现的回退方案，但不等价于完整 Skill：指令文件的名称、作用域和加载时机仍取决于宿主，轻量规则也没有完整的上下文分类和按需扫描器。

<a id="usage"></a>

## 使用

支持隐式匹配的宿主可根据 `description` 自行选择该 Skill，但发现不等于激活。日常可以直接说“交付验收一下”“使用交付门禁”或“检查最终交付”。它会根据影响、不确定性和已有证据选择足够的少量检查，并优先给出通过、有条件通过或未通过。Codex 的原生显式语法是 `$delivery-gate`，Claude Code 是 `/delivery-gate`；其他宿主按其当前方式选择 Skill，不要仅从输出措辞推断已经激活。
...
- 只有实际使用了独立上下文并验证其隔离时，才能声称经过独立验证；普通任务不要求额外 Agent。
